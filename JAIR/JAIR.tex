...
Approaches using NNs divided into two broad classes.

1) Symbolic representations state tracking / reducing the domain
 - much of the challenge is small data regimes. For example, Wen collected data by crowdsourcing, but this only resulted in XX. 
- a typical domain semantic state is very smal - 64 acts? 
- NNs used for predicting the representation given the previous states and some incoming language (Henderson, Mrksic) or generating language from it ()
- evaluation is somewhat easier: can (as well as BLEU) ask users whether it conforms to the DA
- can be harder to get the data
...
\paragrarph{goal-driven models}

\paragraph{data-driven models}

\subsection{Datasets}
brief summary of Iulian's paper and then point to that
 
\subsection{Evaluation issues} 

\subsection{Future of work}

In many ways both approaches are necessary. One ensures the benefits of NNs can be immediately felt. Other follows the philosophy of NNs more closely and could lead to the long-term solution. 
